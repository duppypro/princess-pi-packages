...
**This covers the built commands only** (`serve`, `wtft`, `yada`): the ones in `package.json`'s
`bin` map. The twelve **workflow shell scripts** (`pr-open`, `git-checkpoint`, `repo-gate`, …)
