...
| **Gate** | a tenant's access policy: `"access"` (Cloudflare Access) or `"public"` (open, and owns its own auth) | the *policy*. The Cloudflare resource implementing it is an **Access application**. A `"access"` gate does two jobs at two scopes — account-wide **authentication** (the identity session) and per-tenant **authorization** (the allow-list). Only the second is per-sub-domain, so a gated tenant is **not** the same thing as a login prompt: `serve-standard.md` §8.7 |
| **Identity session** | the account-scoped proof of *who a visitor is*, established once by email-OTP and held on the team domain (`princess-pi.cloudflareaccess.com`), ~24h | shared by **every** tenant in the account, so a visitor who signed in anywhere opens a newly published tenant with no challenge — **but only one they are allow-listed on.** It carries *identity*, never *access*: an identity absent from a tenant's `.serve-acl` gets that tenant's login page and is sent no code, in the same browser and minute that a permitted tenant serves it silently. Never verify a gate from a signed-in browser — `serve-standard.md` §7.6, §8.7, #329 |
...
| **Reap** | deleting ingress rules whose local origin no longer answers | runs on every `serve` invocation. Gated on two facts: the hostname is fronted by a `serve `-prefixed Access application, **and** its port fails a TCP probe (3 attempts / ~1.5 s since princess-pi-packages #181). It asks *"does anything answer here"*, never *"is this one of ours"* — so a listening service tenant is kept. **Residual gap (liveness):** a tenant that is down across the whole probe window (a slow `systemctl restart`) still reads dead. Correct service liveness is a systemd question answered from the manifest `unit` — `serve-standard.md` §9. **Residual gap (ownership):** the ownership half of the test is *"is there a `serve `-prefixed Access application"*, which a `gate = "public"` tenant can never satisfy — §2.3 gives it no Access application — so its ingress is **never** reaped and can outlive its origin onto a reused port. `serve-standard.md` §8.6, #294. *Earlier text here claimed reap matched `ps aux`; it never did — see #9.* |
