...
- **Commits on `main` are blocked, not just pushes (#301, btw#21).** `git commit`, `merge`,
  ...
  **empty**. The lift therefore records that empty name — recording the *target* blocked the
  ...
  commit; git push` keeps its lift); lifts are stored per repo — keyed since #421 on the repository's **identity** (`git rev-parse --absolute-git-dir`) rather than on the words that named it, because `git --git-dir=.git checkout -b x && git commit` spells one repo two ways and the spelling-keyed table failed **both** directions: a lift filed under one key and missed under the other re-read `main` and blocked a legal line, while a `--git-dir` switch *to* `main` was invisible to the bare `commit` that followed it and the #301 gate never fired. `--absolute-git-dir` and not `--git-common-dir`: a linked worktree keeps its own key, since a lift in a worktree is not a lift in the main clone. A directory git cannot identify falls back to the old syntactic key **and marks the whole line degraded**: every later lookup that misses answers *unknown* rather than "whatever the repo is on right now". Resolution failing at the store but not the lookup (or the reverse) would otherwise put the two on different keys, and that miss reads the **pre-switch** branch — fail-open, the one direction this fallback must never take. The cost is over-blocking a line whose earlier sub-command named a directory git cannot identify, which is a line whose earlier sub-command failed anyway; and `--abort`/`--ff-only`
...
**No argument is ever silently dropped (#367).** #362 closed the unknown-*flag* hole and left three
...
- **Unexpected positionals are refused, naming the argument** (exit `2` everywhere;
  ...
  than commit and push its first word.
- **`-h`/`--help`/`--version` are terminal** — they take no other arguments. `pr-cleanup --version
  extra-arg` printed the path and exited `0`, ignoring the extra word while `pr-cleanup`'s own
  header called extra arguments a usage error. The scan stops at `--`, so a value that legitimately
  reads `--version` stays reachable.
- **A flag's value that is itself flag-shaped is refused** unless it follows `--`. `pr-reject -b
  --json` took `--json` as the branch and went on to run live `gh repo view` / `gh pr list --head
  "--json"`; `repo-gate --policy --json` searched for a policy file named `--json` and then blamed a
  missing policy file. `pr-threads --resolve` has guarded this since PR #314; the rest of the family
  now does too.

The guard is duplicated per script by necessity, not preference: `install-workflow-tools` copies each
one to `~/bin` as a standalone file, so there is no shared library for them to source. The full
rationale lives once, in `bin/pr-open`; the others carry a two-line pointer to it.
...
| `pr-open [--reviewed]` | Discovers branch from cwd → fetches (`--prune`) → pushes only if local/remote shas differ, refusing a diverged branch rather than force-pushing (see [Git guardrails](#git-guardrails)) → pre-checks → `pr-review` → `gh pr create`. Takes no arguments other than `--reviewed` (#377); a branch name is refused (exit 2, #367), not ignored — until then `pr-open <branch>` opened the *current* branch's PR. To open another branch's PR, run it from that branch's worktree. Review findings mean **exit 7 and no PR**; zero review coverage (`pr-review` exit 8, #401) also means **exit 8 and no PR**; `--reviewed` overrides both, since it skips `pr-review` entirely |
| `pr-review [--base <ref>] [--json] [--quiet]` | Three-lens review of the branch diff before the PR exists (correctness / reasoning / contract). Exit 7 on findings and `pr-open` refuses to create the PR; exit 8 (#401) when **zero** lenses complete — every one timed out, errored, or answered unusably — or (#412) when the collector cannot write its results, its summary cannot be read back, or `--json` cannot read the log back, each of which leaves the run's coverage unknown rather than zero; the two are one answer, because a caller can act on neither — and `pr-open` refuses on that too, rather than reading "reviewer ran and found nothing" and "reviewer never actually looked" as the same clean exit. **Fails open only when the reviewer itself is missing** (`claude`/`python3` never on `PATH`, exit 0) — a broken or timed-out lens that leaves at least one lens completed still falls through to 0 (clean) or 7 (findings); the incident that closed this gap was PR #393, where all three lenses timed out, the run exited 0 with `lensesRan: 0`, `pr-open` opened the PR with no message, and a hand re-run of the same commit found 13 findings, three of them real. The `COVERAGE INCOMPLETE` warning on stderr is still suppressed by BOTH `--quiet` and `--json` (it is one of several `⚠️` lines the script emits, and the stdout line naming how many lenses ran is suppressed the same way), so a caller wanting per-lens detail still reads `lensesRan`/`status` from the JSON rather than the exit code alone — but the exit code itself now tells "clean" from "did not run" apart, which it could not before #401. Environment: `PR_REVIEW_TIMEOUT` (per-lens ceiling, default **600s** — a value that is not a positive whole number is exit 2 before any work; a 30s `--kill-after` grace follows expiry, and clustering carries the same pair, so MODEL-CALL wall clock is at most 2*(ceiling+30) — the JSON scans afterwards sit outside every timeout, and `MAX_FAILED_CANDIDATES` bounds only the rescan of unparseable `{` candidates — the envelope is `json.load`ed whole and a response with no `{` is traversed end to end, both bounded by response size, which pr-review does not cap. The reviewer's own output limit is the real ceiling there, and it is `claude`'s, not ours), `PR_REVIEW_MODEL` (default `claude-sonnet-5`), `PR_REVIEW_LOG_DIR` (outranks `XDG_STATE_HOME` and `HOME`), `PR_REVIEW_NO_CLUSTER=1`. `--json`/the log is schema `pr-review/run@1`: alongside `status`, `lenses`, `lensesRan` and `findings`/`distinct` it carries `lensTimeoutSec`, `lensSeconds` (per lens, seconds) and, on each `failedLenses` entry, `cause` (`timeout` \| `killed` \| `failed` \| `unknown` \| `unreadable`), plus `why`/`stderr`, and `exitCode`/`elapsedSec` where they were recorded — **either may be `null`**, and `unreadable` entries carry `raw` instead (#403). `cause` comes from `timeout -v`, which announces on its own stderr when **it** sent the signal and is silent otherwise: that announcement is the only evidence of expiry, and `timeout` is the sole producer of it. So `timeout` means the marker was present; `137` without it is `killed` (128+9 — a signal from outside, an OOM kill being the usual one); `124` without it is `failed`, because 124 is then just the reviewer's own exit code that happens to match the one `timeout` uses; no recorded exit code is `unknown`. `elapsedSec` is recorded for every lens and is the number to set a ceiling by, but it classifies nothing — a duration near the ceiling was only ever circumstantial. The **six** `emit_status` paths emit a different, smaller object with **no** `schema` key and a `message` key. A missing `schema` means **this run could not relay a full document** — which since #412 is no longer the same statement as "no lens ran", and a caller that short-circuits on it will read a completed-but-unrelayable review as one that never started. Branch on `status`, never on the absence of `schema` alone, because the six are not one outcome: `no-changes` is a **completed** run (repo found, base resolved, `git diff` ran and was empty — exit 0, nothing to review, a success); `reviewer-unavailable` started and failed open; `diff-failed` started and the diff failed; `not-a-repo` and `precondition` never started; and `reviewed-none` (#412) is the one that may in fact have run — the collector could not write its results, its summary could not be read back, or `--json` could not read the log back, so a full document may exist on disk and simply be unrelayable. It exits 8 and means coverage **unknown**, which `pr-open` refuses on exactly like zero coverage. (The same `status` on a schema-CARRYING document keeps its #401 meaning: every lens failed.) Reading `no-changes` as a startup failure is the same "could not look" / "found nothing" merge this gate exists to keep apart. Exit 3 does **not** predict whether JSON appears: the protected-branch and no-base preconditions and the failed diff go through `emit_status` and do emit the small object, while the environment failures after that point (unsafe `TMPDIR`, an uncreatable log directory or log file, an unwritable work dir) print stderr and emit nothing — as do usage errors. Handle an **empty stdout on any nonzero exit** rather than mapping a code to a shape. Each run logs to `${XDG_STATE_HOME:-$HOME/.local/state}/pr-review/<repo>/` — outside any repo, so `git add -A` cannot commit it, and 0600 in a 0700 directory because the log carries findings and clipped reviewer output (the diff itself is never written to it — the base/head shas are, deliberately). `<repo>` is resolved from `git rev-parse --path-format=absolute --git-common-dir`'s parent basename, not `--show-toplevel`'s (#406): `--show-toplevel` inside a worktree is the WORKTREE path, and `pr-open` is documented as run from exactly that layout (`<repo>/.claude/worktrees/<branch>/`), so the old resolution scattered one log directory per branch — orphaned the moment `pr-cleanup` deletes the worktree — instead of grouping every branch's review of one repo together. `--git-common-dir` resolves to the MAIN clone's `.git` from both a worktree and the main clone itself, so the same `<repo>` directory is used either way. Existing per-branch directories from before #406 are not migrated — they are simply not written to again. `PR_REVIEW_LOG_DIR` pointing inside the work tree is refused up front (exit 3): run logs carry findings and reviewer output, and `git add -A` would commit them. A log directory any other account can WRITE — it or any directory above it, though a sticky ancestor such as `/tmp` is exempt — is refused outright and the run degrades to a private temporary one, named on stderr; **the directory you configured then holds nothing**, which is the case to know about when you go looking for a run — in such a directory the file mode guarantees nothing, since the log can be swapped for a symlink between creation and write |
...
| `iarts-mirror` | Refreshes the private bare mirrors of the never-pushed `iarts/local` branches in the client clones (`robotic_hardware`, `rusty-robots`) — the branches the `.iarts` convention keeps out of the client's `origin`, which therefore exist only in the clone and in `~/git-remotes/<repo>.git`. Run it before ending a session in a client clone. Fetches **from** the clone **into** the bare mirror rather than pushing: a fetch names its source as a path and has no concept of `origin`, so it cannot be mis-aimed at the client repo, whereas a push has to be aimed. Then refreshes the clone's own `refs/remotes/mirror/*` (#384) — git updates remote-tracking refs only in the repository that runs the transfer, so before that fix the prompt reported mirrored work as unmirrored and the drift grew with every commit. Takes no arguments (refused, exit 2). Output is headerless whitespace records `repo branch sha status`, where `status` is `updated`, `unchanged`, `rewritten`, `no-clone`, `no-branch` or `fetch-failed`. `updated`/`unchanged`/`rewritten` all mean the branch reached the mirror and may carry a `-refstale` suffix, meaning the mirror is correct but this clone's view of it is not — so an exact match on `updated` reads a half-done run as incomplete. `rewritten` says an amend/rebase/reset moved the branch off the tip the mirror held; the displaced tip stays reachable via the mirror's reflog, which the script configures to never expire (`core.logAllRefUpdates`, `gc.reflogExpire*`). When that retention cannot be set the run does **not** force: ordinary commits still mirror as `updated`, and a rewrite reports `fetch-failed` rather than overwriting a tip nothing could recover. Roots override with `IARTS_MIRROR_ROOT` / `IARTS_CLONE_ROOT`. Exits 0, 1 if any repo failed, or 2. |
...
| 8 | **review coverage** failure — `pr-review` ran but **zero** lenses completed (every one timed out, errored, or answered unusably), so `pr-open` did not create the PR (#401). Emitted only by `pr-review` and `pr-open`. Distinct from 7 the same way 5 is distinct from 6: 7 means the review ran and found a problem, 8 means it never actually looked — "found nothing" and "looked at nothing" are different facts, and PR #393 is the incident that collapsing them into one exit-0 path produced (a hand re-run of the same commit then found 13 findings, three of them real). Same `--reviewed` escape hatch as 7, since `--reviewed` skips `pr-review` outright. A **partial** run — at least one lens completed — is not this code; it still falls through to 0 (clean) or 7 (findings). |
