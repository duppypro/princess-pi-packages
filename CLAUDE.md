...
| `iarts-mirror` | Backs up the client clones' `iarts/local` branches into `~/git-remotes/<repo>.git`. Those branches never reach the client's `origin`, so the mirror is their only second copy — run it before ending a session in `robotic_hardware` or `rusty-robots`. Pulls into the mirror instead of pushing, so it has no way to name the client remote at all. Records are `repo branch sha status`; treat any status other than a bare `updated`/`unchanged` as work not yet safe |
