...
| `pr-review [--base <ref>] [--json] [--quiet]` | Reviews the branch diff in three parallel lenses (correctness, reasoning, contract) before a PR exists; exit 7 on findings, and **fails open** — a missing, broken, or **timed-out** reviewer still opens the PR (#377), so exit 0 can also mean zero coverage. Logs each run outside the repo, under `PR_REVIEW_LOG_DIR` if set, else `${XDG_STATE_HOME:-$HOME/.local/state}/pr-review/<repo>/`, 0600 because the log holds the branch diff. `PR_REVIEW_TIMEOUT` is the per-lens ceiling (default **600s**, a positive whole number or exit 2); `PR_REVIEW_MODEL` the model (default `claude-sonnet-5`); `PR_REVIEW_NO_CLUSTER=1` skips the clustering pass. Schema `pr-review/run@1` — read coverage from `lensesRan`/`failedLenses[].cause` (`timeout` \| `killed` \| `failed` \| `unknown`, with `exitCode`/`elapsedSec`) and `lensSeconds`/`lensTimeoutSec` (#403), never from the printed sentence |
