...
| `iarts-mirror` | Backs up the client clones' `iarts/local` branches into `~/git-remotes/<repo>.git`. Those branches never reach the client's `origin`, so the mirror is their only second copy — run it before ending a session in `robotic_hardware` or `rusty-robots`. Pulls into the mirror instead of pushing, so it has no way to name the client remote at all. Records are `repo branch sha status`; `updated`/`unchanged` **and their `-refstale` variants** all mean the branch reached the mirror — a `-refstale` suffix reports only that this clone's view of the mirror is stale, so investigate it but do not re-run for safety. Any OTHER status means work not yet safe |
