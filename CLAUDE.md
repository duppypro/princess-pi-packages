...
| `pr-review [--base <ref>] [--json]` | Reviews the branch diff in three parallel lenses (correctness, reasoning, contract) before a PR exists; exit 7 on findings, and **fails open** — a missing or broken reviewer still opens the PR (#377). Logs each run outside the repo, under `$XDG_STATE_HOME/pr-review/<repo>/` |
