...
| `repo-gate [<repo>...]` | Reports how each repo's live branch protection AND the policy's bot login's write access (`.bot_login`) differ from `docs/repo-policy.json` (exit 6 = drift). Report-only; `--remedy <repo>` prints both fixes, `docs/repo-gate-apply.md` teaches an agent to run them |
