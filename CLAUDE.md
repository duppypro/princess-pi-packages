...
| `pr-open [--reviewed]` | Pushes only if needed, refusing a diverged branch → pre-checks → `pr-review` → `gh pr create` (the one command to ship). Findings mean **exit 7 and no PR** (#377); zero review coverage means **exit 8 and no PR** (#401); `--reviewed` opens anyway, either way. Takes no other arguments — it opens the PR for the worktree it runs in, and a branch name is refused, not ignored (#367) |
| `pr-review [--base <ref>] [--json] [--quiet]` | Reviews the branch diff in three parallel lenses (correctness, reasoning, contract) before a PR exists; exit 7 on findings, exit 8 when **zero** lenses complete (#401 — "found nothing" and "looked at nothing" are different facts). **Fails open only when the reviewer itself is missing** — `claude`/`python3` never on `PATH` — not when it is broken or every lens times out; a partial run (at least one lens completed) still falls through to 0 or 7. Logs each run outside the repo, under `PR_REVIEW_LOG_DIR` if set — a value inside the work tree is **refused** (exit 3), because `git add -A` would commit it — else `${XDG_STATE_HOME:-$HOME/.local/state}/pr-review/<repo>/`, where `<repo>` is the MAIN clone's directory name even when run from a worktree (#406). A directory another account can write is not used at all: the run says so and degrades to a private temp dir, so the named path may hold nothing. 0600 — it holds findings and clipped reviewer output, not the diff itself. `PR_REVIEW_TIMEOUT` bounds every `claude` call including clustering (default **600s**, a positive whole number or exit 2); `PR_REVIEW_MODEL` the model (default `claude-sonnet-5`); `PR_REVIEW_NO_CLUSTER=1` skips the clustering pass. Schema `pr-review/run@1` — read coverage from `lensesRan`/`failedLenses[].cause` (`timeout` \| `killed` \| `failed` \| `unknown` \| `unreadable`; `exitCode`/`elapsedSec` where recorded, either possibly `null`) and `lensSeconds`/`lensTimeoutSec` (#403), never from the printed sentence |
