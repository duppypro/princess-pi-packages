...
var SUBDOMAIN_MAP_PATH = path.join(os.homedir(), ".pi-certs", "serve-subdomains.json");
...
async function checkLabelAvailable(cf, label, activeLabels) {
  ...
  try {
    ...
    if (FALLBACK_RESERVED.has(label))
...
` + buildNoDirHint());
  ...
  for (const rawDir of dirs) {
    ...
    const activeServers = await discoverServers();
    const existingServer = activeServers.find((s) => path4.resolve(process.cwd(), s.dir) === targetDir && !!s.isLive === !isStatic);
    if (existingServer) {
    ...
    const runnerPath = path4.resolve(__dirname2, "../extensions/lib/serve/run-live-server.js");
