 ...
 * longer exercise the plain thread-count behaviour this suite is for. An
 * empty `reviews.nodes` keeps review coverage in the advisory state (no
 * reviews ever → exit 0 when threads are clean), which is what this suite's
 * exit-code assertions already assume.
 */
