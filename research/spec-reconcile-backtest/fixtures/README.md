...
|---|---|---|---|
| `host/git-projects-CLAUDE.md` | `~/git-projects/CLAUDE.md`, "Git & GitHub Etiquette" subsection — the subsection is 25 lines (104-128); **lines 104-127 are staged verbatim** and line 128, the trailing *Model Routing for Spawned Agents* bullet, is elided as unrelated to any tool this corpus audits. The staged file prepends a title line and a blank, so it is 26 lines long | `2026-08-19T16:01:47Z`, from the host's own `.bak` snapshot | The state immediately **before** #381 corrected it. Scored as **F5** in `RUBRIC.md`. The Git Guardrails bullet — the F5 claim — is byte-for-byte |
