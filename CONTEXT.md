...
**Serve ACL**:
The `.serve-acl` file in a served directory. One email per line (or `@domain` for a whole domain). Parsed by `parseAclFile()` and fed into the Access application's allow policy on publish. It is the **only** thing that isolates one sub-domain from another — the login prompt does not (`serve-standard.md` §8.7). When missing it is auto-seeded from `~/.config/princess-pi/default-acl`, else the repo's `git config user.email`. **The lookup is one directory deep**, plus that global default — a `.serve-acl` in a parent directory does not apply. In particular `~/.serve-acl` governs nothing except `serve ~` itself, where `$HOME` *is* the served directory; it was deleted from this host on 2026-08-17 (#336) because its comments claimed otherwise. The unioning-up-to-`$HOME` cascade those comments described was written but never shipped — `resolveCascadeAcl()` / `aclSearchPath()` in `research/parked-32-cloudflare-serve/extensions/lib/serve/acl-cascade.js`, tracked by #116.
